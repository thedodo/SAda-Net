...
def RunPred(image_left, image_right, max_disp, save, w_filter):
    
    h,w = image_left.shape
    
    cost_volL, cost_volR = createCostVolAllTogether(image_left, image_right, max_disp)
    
    if(w_filter):
        
        cost_volRfiltered = filterCostVolBilatpyt(cost_volR,image_right)
        cost_volRfiltered = np.squeeze(cost_volRfiltered.cpu().data.numpy())                

        cost_volLfiltered = filterCostVolBilatpyt(cost_volL,image_left)
    ...
    writePFM(out_folder + 'example_sparse.pfm',disp_s.astype(np.float32)) 

    nr_incons = np.count_nonzero(np.isnan(disp_s))
        

            
    return disp_s, dispL, nr_incons
...
def loadTif(tif):
    ...
    im_tif = gdal.Open(tif)
    im_band = im_tif.GetRasterBand(1)
    im_arr = im_band.ReadAsArray()
    im_arr[np.where(np.isnan(im_arr))] = 0.0

    return im_arr


left_image = './sample/example_left.tif'
right_image = './sample/example_right.tif'

image_left = loadTif(left_image)
image_right = loadTif(right_image)

branch.load_state_dict(torch.load('./weights/branch/jack'))
simB.load_state_dict(torch.load('./weights/simB/jack'))


new_disps, disp_final, nr_incons_tot = RunPred(image_left, image_right, 95, True, True)
